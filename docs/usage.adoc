...
In the context of Xp7Deployment, `initContainers` are containers launched before the XP container itself, as part of the deployment lifecycle. This mechanism is particularly useful to set up the environment or parameters for the main application or to ensure all prerequisites are in place before the actual application starts.
...
For instance, a sidecar can be conceived to perform logging, monitoring, synchronization, and background data processing tasks.
...
[#env]
==== env
As for initContainers and sidecars, you can specify environment variables for the XP containers. This can be done by adding an `env` block to the particular `nodeGroup` of the `Xp7Deployment` resource.
