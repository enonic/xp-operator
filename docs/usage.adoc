...
This is the main custom resource that defines your deployment. It is namespaced, so you have to create a namespace for it. Only 1 Xp8Deployment can reside in each namespace.
...
In the Xp8Deployment resource we have names of disks that you can define to be shared, private or non-existent.
...
An `initContainer` block in `Xp8Deployment` has several possible fields:
...
|name
|name of an actual `dir`, `disk` or `volume` name, defined in `Xp8Deployment` resource.
...
[#env]
==== env
In the same way as for initContainers and sidecars, you can specify environment variables for the XP containers. This can be done by adding an env block to the particular nodeGroup of the Xp8Deployment resource.
...
This custom resource manages the configuration of XP nodes. This resource namespaced, and should be created in a namespace that already contains a Xp8Deployment.
...
You can also create Xp8Config files that hold binary data. To do that you have to base64 encode the data and set the `dataBase64` field to true like so:
...
NOTE: It can vary how fast XP registers the Xp8Config changes. It can be instant, but it can also take up to a couple of minutes, depending on your Kubernetes cluster setup.
...
[source,yaml]
----
enonic.cloud/xp8.vhost.mapping.<MAPPING_NAME>.source: /admin
enonic.cloud/xp8.vhost.mapping.<MAPPING_NAME>.target: /admin
enonic.cloud/xp8.vhost.mapping.<MAPPING_NAME>.idproviders: <DEFAULT_IDPROVIDER>,<OTHER_ENABLED_IDPROVIDER>
----


A very important thing to keep in mind is that the annotation `enonic.cloud/xp8.vhost.mapping.<MAPPING_NAME>.source` has to match a defined `spec.rules[?].http.paths[?].path` in the same ingress. That is so the operator knows what node groups it needs to update. That brings us to the second point. The `spec.rules[?].http.paths[?].backend.serviceName` has to match a node group name defined in your Xp8Deployment.
...
metadata:
  ...
  annotations:
    # all hosts mapping
    enonic.cloud/xp8.vhost.mapping.my-mapping-site.source: /
    enonic.cloud/xp8.vhost.mapping.my-mapping-site.target: /site/default/master/homepage

    # specific host mapping
    enonic.cloud/xp8.vhost.mapping.my-mapping-admin.host: my-domain.com
    enonic.cloud/xp8.vhost.mapping.my-mapping-admin.source: /admin
    enonic.cloud/xp8.vhost.mapping.my-mapping-admin.target: /admin
    enonic.cloud/xp8.vhost.mapping.my-mapping-admin.idproviders: system
spec:
  rules:
    - host: my-domain.com
      http:
...
NOTE: Like Xp8Config, changes to virtual hosts can take a couple of minutes to register in XP.

=== Namespaces

It can be desirable to delete all created resources that are associated with an Xp8Deployment once its deleted. That is quite easy to do with this namespace annotation:
...
metadata:
  name: my-namespace
  annotations:
    enonic.cloud/remove.with.xp8deployment: my-deployment
----
...
[source,bash]
----
$ kubectl -n my-namespace get xp8deployments.enonic.cloud
...
[source,bash]
----
$ kubectl -n my-namespace get xp8configs.enonic.cloud
...
[source,bash]
----
$ kubectl -n my-namespace edit xp8configs.enonic.cloud my-config

xp8config.enonic.cloud/my-config edited
----
...
[source,bash]
----
$ kubectl -n my-namespace get xp8configs.enonic.cloud -w
...
In case you want to map all your disks to a single volume, you can change the `Xp8Deployment` resource from the previous example to look like this:
