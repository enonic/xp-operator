...
Note: When deploying xp on an Openshift cluster, it fails to create resources due to security restrictions by Openshift. To be able to run xp on openshift you need to add the following policy to the namespace you are going to deploy xp. That will grant the service account "xp-node-sa" (within the name space for xp deployment)  access to privileged security context configuration. And xp related resource can then run in that name space.  
$ oc adm policy add-scc-to-user privileged -z xp-node-sa -n <xp-deployment-namespace>
...
spec:
  ...
  xpVersion: 7.7.1  # XP version, has to be >=7.7.0
...
my-deployment   true      7.7.1     RUNNING   OK
----
