...
3. Click on "Create" button. This will take you the k8s cluster creation wizard. By default it starts the creation process for "autopilot" k8s cluster. Click on the "Switch to standard cluster" button.  (Note: Current xp operator version works only on "standard" cluster type)
...
XP can run in cluster mode in a GKE cluster. The main differecnce from running a single XP instance is that, xp cluster requires an NFS based shared volume that can be mounted as Read-Write by multiple nodes at a time. 


The standard storage class in `GKE` does not support `ReadWriteMany` mode. Most storage classes that support `ReadWriteMany` access mode have also a restriction in changing the modified time of files that XP requires. At the time of this writing, the only storage classes that can be used to deploy XP in cluster mode are NFS based storage classes with `ReadWriteMany` access mode. 
