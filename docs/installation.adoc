...
`* NFS based storage classes in AKS and GKE have minimum size requirements which can be very high depending on your storage requirement. Please refer to the respective cloud provider's site for more information.`

Note: The ´Filestore CSI driver´ and ´Azurefile CSI NFS´ storage classes are native to the managed k8s clusters GKE and AKS respectively and are available on k8s clusters without additional setup. However, for Amazon EKS, you need to create the ´EFS´ storage class. See our <<efs#,EFS storage class>> guide for more information. 
