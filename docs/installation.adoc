...
NB: Azurefile storageclass on AKS supports RWM access mode but xp-operator fails to work on that and we recommend usage of NFS on AKS.  
== Installation
...
* Remove xp7deployment and all related resources like PVCs, cluster role and cluster role bindings. If you used a separate namespace for your xp7deployment with no other k8s resource, it will be easier to delete the entire namespace. 
