...
WARNING: The operator will create a unique namespace per XP deployment. Never install the operator and an XP7 deployment into the same namespace.


== Shared storage class

XP7 clusters require the availability of a shared filesystem - NFS style. Most cloud providers offer out-of-the-box storage classes for this purpose. When setting up your cluster, you should already have identified the available storage classes.

If you are running an ´on-premiser´ k8s cluster, or for other reasons don't want to use the standard NFS storage classes, we recommend setting up your own <<nfs#,provision your own NFS storage>>.
...
WARNING: Make sure your the storage classes are compatible with your Kubernetes cluster and cloud provider.
...
The best way of verifying that everything is working, is by creating a basic xp7deployment.

NOTE: The example below will create a namespace, a single-node XP7 server, pre-install selected applications, and configure an ingress.
...
With the XP running, we can try accessing the Admin - even without setting up an ingress.
