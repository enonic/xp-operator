package com.enonic.ec.kubernetes.operator.commands;

import java.util.List;

import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import io.fabric8.kubernetes.api.model.Namespace;
import io.fabric8.kubernetes.api.model.ObjectMeta;
import io.fabric8.kubernetes.api.model.OwnerReference;
import io.fabric8.kubernetes.client.KubernetesClient;

import com.enonic.ec.kubernetes.common.commands.Command;

import static com.enonic.ec.kubernetes.common.assertions.Assertions.assertNotNull;

public class CommandApplyNamespace
    implements Command<Namespace>
{
    private final Logger log = LoggerFactory.getLogger( CommandApplyNamespace.class );

    private final KubernetesClient client;

    private final OwnerReference ownerReference;

    private final String name;

    private CommandApplyNamespace( final Builder builder )
    {
        client = assertNotNull( "client", builder.client );
        ownerReference = assertNotNull( "ownerReference", builder.ownerReference );
        name = assertNotNull( "name", builder.name );
    }

    public static Builder newBuilder()
    {
        return new Builder();
    }

    @Override
    public Namespace execute()
    {
        log.info( "Creating/Updating Namespace '" + name + "'" );

        Namespace namespace = client.namespaces().withName( name ).get();
        if ( namespace != null )
        {
            log.debug( "Namespace exists, should we update owner reference?" );
            boolean addOwnerReference = true;
            for ( OwnerReference existingOwnerReference : namespace.getMetadata().getOwnerReferences() )
            {
                if ( existingOwnerReference.getUid().equals( ownerReference.getUid() ) )
                {
                    addOwnerReference = false;
                    break;
                }
            }

            if ( addOwnerReference )
            {
                log.debug( "Adding another owner reference to namespace" );
                namespace.getMetadata().getOwnerReferences().add( ownerReference );
            }
            else
            {
                log.debug( "Not updating owner reference" );
            }
        }
        else
        {
            log.debug( "Creating new namespace object" );
            ObjectMeta metaData = new ObjectMeta();
            metaData.setOwnerReferences( List.of( ownerReference ) );
            metaData.setName( name );
            namespace = new Namespace();
            namespace.setMetadata( metaData );
        }

        return client.namespaces().createOrReplace( namespace );
    }

    public static final class Builder
    {
        private KubernetesClient client;

        private OwnerReference ownerReference;

        private String name;

        private Builder()
        {
        }

        public Builder client( final KubernetesClient val )
        {
            client = val;
            return this;
        }

        public Builder ownerReference( final OwnerReference val )
        {
            ownerReference = val;
            return this;
        }

        public Builder name( final String val )
        {
            name = val;
            return this;
        }

        public CommandApplyNamespace build()
        {
            return new CommandApplyNamespace( this );
        }
    }
}
