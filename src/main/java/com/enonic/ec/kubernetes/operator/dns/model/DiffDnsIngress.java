    ...
    extends Diff<DnsIngress>
{
    ...
    {
        List<Domain> oldDomains = oldValue().map( DnsIngress::domains ).orElse( Collections.EMPTY_LIST );
        List<Domain> newDomains = newValue().map( DnsIngress::domains ).orElse( Collections.EMPTY_LIST );
    ...
    {
        List<String> oldIps = oldValue().map( DnsIngress::ips ).orElse( Collections.EMPTY_LIST );
        List<String> newIps = newValue().map( DnsIngress::ips ).orElse( Collections.EMPTY_LIST );
