...
import com.enonic.kubernetes.operator.xp8deployment.Predicates;
...
/**
 * This operator class collects all Xp8Configs and merges them into the nodegroup ConfigMaps
 */
@Singleton
    ...
    implements Runnable, ApplicationEventListener<ServerStartupEvent>
{
    ...
    public void run()
    {
        final Set<String> activeNamespaces = searchers.xp8Deployment()
            .stream()
    ...
    private void handle( final ConfigMap configMap )
    {
        ...
        // Collect all data from Xp8Config that has nodeGroup 'all' or '<nodeGroup>'
        List<Xp8Config> configs = searchers.xp8Config().stream().
