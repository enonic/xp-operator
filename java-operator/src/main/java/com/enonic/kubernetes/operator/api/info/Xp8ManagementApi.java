...
import static com.enonic.kubernetes.operator.xp8deployment.Predicates.running;
import static com.enonic.kubernetes.operator.xp8deployment.Predicates.withNodeGroup;

@Controller("/apis/operator.enonic.cloud/v1")
public class Xp8ManagementApi
{

    public static Logger logger = LoggerFactory.getLogger( Xp8ManagementApi.class );
    ...
    {
        Optional<Xp8Deployment> xp8Deployment = searchers.xp8Deployment().find( inNamespace( namespace ), withName( name ) );

        if ( xp8Deployment.isEmpty() )
        {
            throw k8sException( 404, String.format( "No Xp8Deployment in namespace '%s' with name '%s' not found", namespace, name ) );
        }

        if ( !withNodeGroup( nodeGroup ).test( xp8Deployment.get() ) )
        {
            throw k8sException( 404,
                                String.format( "No Xp8Deployment in namespace '%s' with name '%s' does not have nodegroup %s", namespace,
                                               name, nodeGroup ) );
        }

        if ( !running().test( xp8Deployment.get() ) )
        {
            throw k8sException( 503, "Xp8Deployment not running" );
        }

        return xpClientCache.getClient( XpClientCacheKeyImpl.of( namespace, name, nodeGroup ) );
    }

    @Get("/xp8/{namespace}/{name}/{nodegroup}/mgmt/repo/snapshots/list")
    @Produces("application/json")
    public Xp8MgmtSnapshotsList snapshotList( @PathVariable("namespace") final String namespace, @PathVariable("name") final String name,
                                              @PathVariable("nodegroup") final String nodeGroup )
        throws KubernetesClientException
    {
        try
        {
            final Xp8MgmtSnapshotsList xp8MgmtSnapshotsList = getClient( namespace, name, nodeGroup ).snapshotsList();
            logger.warn(new ObjectMapper().writeValueAsString( xp8MgmtSnapshotsList ));
            return xp8MgmtSnapshotsList;
        }
        catch ( Exception e )
        {
    ...
    @Get("/xp8/{namespace}/{name}/{nodegroup}/mgmt/idproviders/list")
    @Produces("application/json")
    public List<Xp8MgmtIdProvider> idProvidersList( @PathVariable("namespace") final String namespace, @PathVariable("name") final String name,
                                                    @PathVariable("nodegroup") final String nodeGroup )
        throws KubernetesClientException
    {
        try
    ...
    @Get("/xp8/{namespace}/{name}/{nodegroup}/mgmt/content/projects/list")
    @Produces("application/json")
    public List<Xp8MgmtProject> projectsList( @PathVariable("namespace") final String namespace, @PathVariable("name") final String name,
                                              @PathVariable("nodegroup") final String nodeGroup )
        throws KubernetesClientException
    {
        try
    ...
    @Get("/xp8/{namespace}/{name}/{nodegroup}/mgmt/webapps/list")
    @Produces("application/json")
    public List<Xp8MgmtWebapp> webappsList( @PathVariable("namespace") final String namespace, @PathVariable("name") final String name,
                                            @PathVariable("nodegroup") final String nodeGroup )
        throws KubernetesClientException
    {
        try
    ...
    @Get("/xp8/{namespace}/{name}/{nodegroup}/mgmt/apps/list")
    @Produces("application/json")
    ...
    @Post("/xp8/{namespace}/{name}/{nodegroup}/mgmt/apps/install")
