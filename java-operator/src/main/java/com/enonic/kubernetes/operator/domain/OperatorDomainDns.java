    ...
    extends InformerEventHandler<Domain>
{
    ...
    @ConfigProperty(name = "dns.lb.cname")
    Optional<String> cnameDomain;
    ...
    private void lookupDomain( final Domain domain )
    {
        ...
        {
            updateStatus( domain, DomainStatus.State.ERROR, e.getMessage(), false );
            return;
        }
    ...
    private void syncDnsRecords( final DomainConfig config, final Domain domain, final boolean delete )
    {
        ...
        if ( ips.size() == 0 && cnameDomain.isEmpty() )
        {
        ...
        final DnsRecordManager dnsRecordManager = new DnsRecordManager( dnsRecordService, config, domain );
        ...
        {
            // sync records
            errorMessage = cnameDomain.isEmpty()
                ? dnsRecordManager.syncRecords( records, ips, "A", clusterId )
                : dnsRecordManager.syncRecords( records, List.of( cnameDomain.get() ), "CNAME", clusterId );
        }
