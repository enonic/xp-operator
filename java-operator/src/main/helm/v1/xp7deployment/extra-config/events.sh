...
function sendEvent() {
    (cat - | curl -f -s -X POST -H "Accept: application/json" -H "Content-Type: application/json" -H "Authorization: Bearer $K8S_TOKEN" --cacert /run/secrets/kubernetes.io/serviceaccount/ca.crt -d @- https://kubernetes.default.svc.cluster.local/api/v1/namespaces/`cat /run/secrets/kubernetes.io/serviceaccount/namespace`/events > /dev/null || (echo "failed" && false)) && echo "success"
